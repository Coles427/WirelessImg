...
void writeCommand(unsigned char c) {
    ...
    MAP_SPICSEnable(GSPI_BASE); //Enable CS on SPI module
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0); //Set D/C low for writing command
    GPIOPinWrite(GPIOA1_BASE, 0x01, 0); //Set CS for OLED low
    MAP_SPIDataPut(GSPI_BASE,c); //Send char over SPI
    MAP_SPIDataGet(GSPI_BASE,&ulDummy); //Clean up
    GPIOPinWrite(GPIOA1_BASE, 0x01, 0x01); //Set CS for OLED high
    MAP_SPICSDisable(GSPI_BASE); //Disable CS on SPI module
}
//*****************************************************************************

void writeData(unsigned char c) {
       unsigned long ulDummy;
       MAP_SPICSEnable(GSPI_BASE); //SPI module chip select
       GPIOPinWrite(GPIOA3_BASE, 0x80, 0x80); //Set D/C high for writing data
       GPIOPinWrite(GPIOA1_BASE, 0x01, 0); //Set CS for OLED low
       MAP_SPIDataPut(GSPI_BASE,c); //Send char over SPI
       MAP_SPIDataGet(GSPI_BASE,&ulDummy); //Clean up
       GPIOPinWrite(GPIOA1_BASE, 0x01, 0x01); // SET CS for OLED high
...
//Only change here was setting reset to pin 1
  volatile unsigned long delay;

  GPIOPinWrite(GPIOA1_BASE, 0x10, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA1_BASE, 0x10, 0x10);	// RESET = RESET_HIGH
